...
def decrypt_file(input_file, output_file, key):
    ...
    if recalculated_hash != data_hash:
        raise ValueError("L'intégrité du fichier a été compromise.")
...
if __name__ == "__main__":
    choice = input("Voulez-vous (E)ncrypter ou (D)écrypter un fichier ? ").strip().lower()
    
    if choice == 'e':
        input_file = input("Entrez le chemin du fichier à chiffrer : ")
        output_file = input("Entrez le chemin du fichier de sortie : ")
        key, timestamp = generate_key()
        encrypt_file(input_file, output_file, key)
        print(f"Fichier chiffré avec succès. Conservez cette valeur pour le déchiffrement : {timestamp}")
    elif choice == 'd':
        input_file = input("Entrez le chemin du fichier à déchiffrer : ")
        output_file = input("Entrez le chemin du fichier de sortie : ")
        ...
        print("Choix invalide. Veuillez entrer 'E' ou 'D'.")
